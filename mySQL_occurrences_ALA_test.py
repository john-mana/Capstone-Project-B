...
#the file must be in the same route
XLSX_PATH = "ALA-LansdowneSouth.xlsx" 
SHEET_NAME = "Sheet1"
OUTPUT_SQL = "ala_import_test.sql"

SOURCE_NAME = "ALA"
DATASET_NAME = "ALA-LansdowneSouth-Flora"
...
if missing:
    raise ValueError(f"Missing required columns in Sheet1: {missing}")

# Build unique taxonomy rows
taxonomy_rows = []
taxonomy_seen = set()
...
# Generate SQL
lines = []
lines.append("/*ALA Sheet1 import SQL Generated from ALA-LansdowneSouth.xlsx*/")
lines.append("-- locally worked to process the ALA to have suitable .sql format. Generative AI was used to code.")
...
for _, row in df.iterrows():
    ...
    scientific_name_value = scientific_name
    ...
    data_generalisation = clean(row[col_data_gen]) if col_data_gen else None
    ...
    lines.append(
        ...
        "NULL, NULL, "
        f"{sql_literal(scientific_name_value)}, "
        f"{sql_literal(lat)}, "
        f"{sql_literal(lon)}, "
        f"{sql_literal(event_date)}, "
        f"{sql_literal(recorded_by)}, "
        "0, "
        f"{sql_literal(data_generalisation)}, "
