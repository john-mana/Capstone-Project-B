...
def load_data():
    """Load the plant traits data from database."""
    print("Loading plant traits data from database...")
    ...
    from models import Species, SpeciesTraitJunction
    
    with app.app_context():
        # Get all species with their traits
        species_data = []
        species_list = db.session.query(Species.scientific_name).all()
        
        for species in species_list:
            species_name = species.scientific_name
            
            # Get all traits for this species
            traits = db.session.query(
            ...
            species_traits = {trait.trait_name: trait.trait_value for trait in traits}
            species_traits['species_name'] = species_name
            
            species_data.append(species_traits)
...
def generate_pdf_report(species_data, detailed_df, fig, analysis_results):
    ...
    output_dir = Path("experiment4_outputs")
    output_dir.mkdir(exist_ok=True, parents=True)
    
    with PdfPages(output_dir / 'fire_traits_analysis_report.pdf') as pdf:
        # Title page
...
def main():
    ...
    if species_data:
        ...
        species_df.to_csv(output_dir / 'fire_species_with_traits.csv', index=False)
        print(f"\nSpecies data saved to: {output_dir / 'fire_species_with_traits.csv'}")
    
    # Save detailed analysis to CSV
    if not detailed_df.empty:
        detailed_df.to_csv(output_dir / 'fire_traits_detailed_analysis.csv', index=False)
        print(f"Detailed analysis saved to: {output_dir / 'fire_traits_detailed_analysis.csv'}")
    
    # Save summary table
    if analysis_results:
        summary_df = create_trait_summary_table(analysis_results)
        summary_df.to_csv(output_dir / 'fire_traits_summary.csv', index=False)
        print(f"Summary table saved to: {output_dir / 'fire_traits_summary.csv'}")
